...
class Bot:
    ...
    def make_guess(self):
        ...
        # Randomly choose 4 different colors but avoid duplicate guesses
        guess = None
        while guess is None or guess in self.previous_guesses:
            guess = self.rng.choices(self.code_colors, k=self.code_length)
            guess = tuple(guess)
        return guess
